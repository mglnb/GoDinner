  ...
  render () {
    return (
      <li className={'restaurant_menu__modal__list__item'}>
        ...
        <input type={'radio'} onClick={() => this.props.removeIndex(this.props.indexTwo)} />
...
const SortableList = SortableContainer(({items, removeIndex, onChangeItem}) => {
  return (
    <ul className={'restaurant_menu__modal__list'}>
      {items.map(({value, description}, index) => (
        <React.Fragment key={`item-${index}`}>
          <SortableItem key={`item-${index}`} index={index} value={value} description={description} onChangeItem={onChangeItem} indexTwo={index} removeIndex={removeIndex} />
        </React.Fragment>
      ))}
    </ul>
  );
